...
function max_finite_field(rows, field::Symbol)
    values = [Float64(getproperty(row, field)) for row in rows
              if isfinite(Float64(getproperty(row, field)))]
    isempty(values) && return NaN
...
function paired_threshold_summary(rows)
    ...
    push_count_summary!(out, rows, :all, :higher_government_net,
        row -> row.higher_government_net)
    push_count_summary!(out, rows, :all, :higher_support_efficiency,
        row -> row.higher_support_efficiency)
    push_count_summary!(out, rows, :all, :higher_fiscal_cost_efficiency,
        row -> row.higher_fiscal_cost_efficiency)
    push_count_summary!(out, rows, :all, :higher_revenue_efficiency,
        row -> row.higher_revenue_efficiency)
    ...
    push_count_summary!(out, both, :both_available, :higher_government_net,
        row -> row.higher_government_net)
    push_count_summary!(out, both, :both_available, :higher_support_efficiency,
        row -> row.higher_support_efficiency)
    push_count_summary!(out, both, :both_available, :higher_fiscal_cost_efficiency,
        row -> row.higher_fiscal_cost_efficiency)
    push_count_summary!(out, both, :both_available, :higher_revenue_efficiency,
        row -> row.higher_revenue_efficiency)
    return out
end
    ...
    for parameter in parameters
        ...
        for level in levels
            ...
            for dimension in (:stronger_material_saving, :lower_service_loss, :higher_government_net,
                :higher_support_efficiency, :higher_fiscal_cost_efficiency,
...
function support_efficiency_summary(rows)
    ...
    for strategy in sort(collect(Set(row.strategy for row in rows)); by = string)
        ...
        push!(out, (
            ...
            mean_support_cost = mean_field(subset, :support_cost),
            mean_fiscal_cost = mean_field(subset, :fiscal_cost),
            mean_virgin_saving = mean_field(subset, :virgin_saving),
            mean_virgin_saving_per_support_dollar =
                mean_finite_field(subset, :virgin_saving_per_support_dollar),
            max_virgin_saving_per_support_dollar =
                max_finite_field(subset, :virgin_saving_per_support_dollar),
            mean_service_loss_per_support_dollar =
                mean_finite_field(subset, :service_loss_per_support_dollar),
            mean_recycled_use_gain_per_support_dollar =
                mean_finite_field(subset, :recycled_use_gain_per_support_dollar),
            mean_pct_virgin_use = mean_field(subset, :pct_virgin_use),
            mean_pct_toaster_service = mean_field(subset, :pct_toaster_service),
        ))
    end
...
function support_efficiency_parameter_summary(rows, parameters)
    out = NamedTuple[]
    for parameter in parameters
        levels = sort(collect(Set(getproperty(row, parameter) for row in rows)))
        for level in levels
            ...
            total = length(level_rows)
            total == 0 && continue
            for strategy in sort(collect(Set(row.strategy for row in level_rows)); by = string)
                subset = [row for row in level_rows if row.strategy === strategy]
                push!(out, (
                    parameter = parameter,
                    level = level,
                    strategy = strategy,
                    count = length(subset),
                    share_within_level = length(subset) / total,
                    mean_policy_magnitude = mean_field(subset, :policy_magnitude),
                    mean_support_cost = mean_field(subset, :support_cost),
                    mean_virgin_saving = mean_field(subset, :virgin_saving),
                    mean_virgin_saving_per_support_dollar =
...
function support_efficiency_two_way_summary(rows, x_parameter::Symbol, y_parameter::Symbol)
    out = NamedTuple[]
    x_levels = sort(collect(Set(getproperty(row, x_parameter) for row in rows)))
    y_levels = sort(collect(Set(getproperty(row, y_parameter) for row in rows)))
    for x_level in x_levels, y_level in y_levels
        ...
        total = length(cell_rows)
        total == 0 && continue
        for strategy in sort(collect(Set(row.strategy for row in cell_rows)); by = string)
            subset = [row for row in cell_rows if row.strategy === strategy]
            push!(out, (
                x_parameter = x_parameter,
                x_level = x_level,
                y_parameter = y_parameter,
                ...
                strategy = strategy,
                count = length(subset),
                share_within_cell = length(subset) / total,
                mean_virgin_saving_per_support_dollar =
                    mean_finite_field(subset, :virgin_saving_per_support_dollar),
                mean_service_loss_per_support_dollar =
                    mean_finite_field(subset, :service_loss_per_support_dollar),
...
if !isempty(support_efficiency_rows)
    ...
    write_rows_csv(joinpath(output_dir, "fiscal_parameter_region_support_efficiency_summary.csv"),
        support_efficiency_summary(support_efficiency_rows))
    write_rows_csv(joinpath(output_dir,
            "fiscal_parameter_region_support_efficiency_parameter_summary.csv"),
        support_efficiency_parameter_summary(support_efficiency_rows, group_fields))
    write_rows_csv(joinpath(output_dir,
            "fiscal_parameter_region_support_efficiency_eol_service_summary.csv"),
        support_efficiency_two_way_summary(support_efficiency_rows, :sigma_eol, :eta_service))
end
...
if !isempty(support_efficiency_rows)
    println("Best support-efficiency summary:")
    for row in support_efficiency_summary(support_efficiency_rows)
        println("  $(row.strategy): count=$(row.count), share=$(row.share), " *
                "mean_saving_per_support=$(row.mean_virgin_saving_per_support_dollar), " *
                "mean_service_loss_per_support=$(row.mean_service_loss_per_support_dollar)")
    end
end
