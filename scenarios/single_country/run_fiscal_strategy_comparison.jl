...
function channel_summary(rows, strategy::Symbol)
    ...
    return (
        ...
        mean_revenue_gain = mean_field(subset, :revenue_gain),
        ...
        mean_virgin_saving_per_fiscal_cost_dollar =
            mean_finite_field(subset, :virgin_saving_per_fiscal_cost_dollar),
        mean_virgin_saving_per_revenue_dollar =
            mean_finite_field(subset, :virgin_saving_per_revenue_dollar),
...
if !isempty(support_efficiency_rows)
    write_rows_csv(joinpath(output_dir, "fiscal_strategy_comparison_support_efficiency.csv"),
        support_efficiency_rows)
    write_rows_csv(joinpath(output_dir, "fiscal_strategy_comparison_support_efficiency_summary.csv"),
...
println("Best support-efficiency strategy count: $(count_by(support_efficiency_rows, :strategy))")
...
println("Higher support efficiency at threshold: $(count_by(frontier_comparison, :higher_support_efficiency))")
if !isempty(support_efficiency_rows)
...
if !isempty(threshold_rows)
    ...
    for row in [channel_summary(threshold_rows, :virgin_material_tax),
        ...
        println(
            ...
            "mean_virgin_saving_per_support_dollar=$(row.mean_virgin_saving_per_support_dollar), " *
